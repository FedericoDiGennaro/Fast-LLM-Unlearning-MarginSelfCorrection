...
def preprocess_tofu_chat_instance(
    ...
    if template_name == "qwen25":
        ...
        raise ValueError(f"Unsupported TOFU chat template: {template_name}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--template_name",
        type=str,
        choices=["llama2", "qwen25"],
        default="qwen25",
        help="TOFU QA chat template. This Qwen-specific copy defaults to qwen25.",
    )
    parser.add_argument(
    ...
    if not quiet_timing:
        ...
        if args.corpus == "tofu":
            print(f"TOFU chat template: {args.template_name}")
    ...
    if args.corpus == "tofu":
        ...
        retain_ids, retain_labels, retain_attention, retain_meta = encode_tofu_pairs(
            tokenizer, retain_pairs, args.max_len, args.template_name
        )
        forget_ids, forget_labels, forget_attention, forget_meta = encode_tofu_pairs(
            tokenizer, forget_pairs, args.max_len, args.template_name
        )
        if not quiet_timing:
    ...
    probe_loop_enabled = stop_criteria_enabled or args.online_fq_probe == "yes" or args.force_probe_loop
    ...
    if probe_loop_enabled:
        start_probe_metrics = early_stop_probe_metrics(
            ...
            need_surrogate_mean=two_stage_surrogate_enabled,
            need_gap_stats=(
                args.force_probe_loop
                or holdout_online_val_enabled
                or args.early_stop_retain_forget_margin_gap_min is not None
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for _ in range(steps_per_epoch):
            ...
            if quiet_timing and args.objective_mode == "forget_margin":
                ...
                if args.objective_mode == "asymmetry_gap_full":
            ...
            if (
                ...
                if criteria_met:
                    ...
                    if args.timing_mode and args.timing_skip_save:
                        saved_format = "skipped_for_timing"
                    else:
                    ...
                    early_stop_payload = {
                        ...
                        "global_step": global_step,
                        "optimizer_step": optimizer_step,
                        "checkpoint_dir": None if (args.timing_mode and args.timing_skip_save) else str(early_stop_dir),
    ...
    if not (args.timing_mode and args.timing_skip_save):
